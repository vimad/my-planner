...
// real name anyway by cross-referencing Atlas's own roster (see
// hooks/useAtlasRosterPeople.ts) - a task assigned to someone added to that
// roster resolves to their name; a real Jira assignee who was never added
// falls back to an "Unmapped" badge, same concept as PlanningView's
// off-roster badge (docs/ui-conventions.md's amber Unmapped convention). If
// this cross-reference proves worth keeping long-term, the real fix is
// adding assigneeDisplayName to AtlasTask/atlasSync.ts so it isn't needed at
// all.
