...
- **"Outside the program" ticket marker** (Atlas's Task Board — a ticket tracked directly rather than pulled in as part of an epic's tree): `border-indigo-400 dark:border-indigo-400/60`, applied as the whole border on a card/chip (`border-2`) or as a left-edge accent on a table row (`border-l-2`) — deliberately border-only, no bg tint and no icon, since the ask was "highlighted border ... without cluttering the card." Indigo is reserved for this one meaning. Reference: `AtlasTaskBoard.tsx`'s `OUTSIDE_PROGRAM_ACCENT`.
