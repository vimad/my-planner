...
- [ ] Reordering triggers a save-on-drop `PATCH /api/sprint-plan-entries/:id` (ticket 13) for the affected entries in that row — no separate save step, no cross-row dragging (each person's row is its own sortable context). For a Split ticket (ticket 23), its dev-row placement and qa-row placement are two independent sortable items — dragging one only ever writes that role's own `devOrder`/`qaOrder`, never the other; a non-split ticket keeps writing plain `order` exactly as before.
