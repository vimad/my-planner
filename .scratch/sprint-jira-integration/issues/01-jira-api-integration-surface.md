...
**Still needs live verification** (can't be resolved from docs alone — do this early in implementation, not later): real board id for "Odyssey"/WOSMVP; the actual `customfield_XXXXX` id and select-type for "Stream"; exact `subtasks` array shape on a real issue; whether `emailAddress`/`parent` are actually populated as expected on a real WOSMVP issue given this org's privacy settings; a smoke test confirming `/search/jql` works on `wealthos.atlassian.net` specifically; and (added while resolving ticket 07) whether the personal API token's account has the **"Browse users and groups"** global permission — required for `GET /rest/api/3/user/search`, which ticket 07's person-mapping flow uses only as an optional autocomplete convenience, so its absence degrades gracefully rather than blocking anything.
