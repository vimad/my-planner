...
- [Dashboard UI](issues/04-dashboard-ui.md) — dense table overview (progress bar + To Do/In Progress/Done/At-risk pills per epic row) with an inline accordion drill-down (task tree expands in normal page flow, not a drawer/modal); recursive indented task rows with a status badge, dates, notes indicator, and blocked-by chips (epic-suffixed when cross-epic); archived epics collapsed behind a "Show N archived" toggle. Three prototyped variants captured on branch `prototype/atlas-dashboard-ui-variants`.
- [Present view UI](issues/05-present-view-ui.md) — compact master/detail: an always-visible left-hand epic strip (mini progress ring, health color, at-risk badge — whole program visible with no scrolling) next to a focused detail pane (status counts, an at-risk/blocked digest, notes) that swaps instantly on click or ↑/↓, no page transition; fully read-only, archived epics excluded entirely (Dashboard-only concept). Four prototyped variants captured on branch `prototype/atlas-present-ui-variants`.
