...
16. As a user, I want to be able to change my mind mid-range-selection by clicking a different first day before committing the second click, so that I'm not stuck with a wrong first pick.
...
- Click sequence:
  ...
  3. Second click on a day (any day, before or after `start`) commits the range: normalize so `start <= end` regardless of click order, call `onSelectRange({ start, end })`.
  ...
  5. Clicking a different day while `start` is provisional but not yet committed replaces the provisional `start` with the new click (per user story 16) rather than treating it as the second click of a range.
  6. Once a range is committed, clicking any day cell again starts a brand-new selection (goes back to step 1) — it does not extend the existing committed range.
...
- Open styling question worth a quick confirmation at implementation time (not a blocking decision): exact color/treatment for "in preview range" vs. "committed range" vs. "today" vs. "office day" so all four remain visually distinguishable when they land on the same or adjacent cells.
